...
import Foundation
let one = Double(endCoord.x) - startCoord.x
let two = Double(endCoord.y) - startCoord.y
let length = sqrt(one * one + two * two)
print(length)

let money = (type: "USD", value: 1.0)
let canadianMoney = (type: "cad", value: money.value * 1.29)
let ausMoney = (type: "aus", money.value * 1.36)

print("\(money.value) \(money.type) is \(canadianMoney.value) \(canadianMoney.type)")
